          ...
          {cartItems.length >= 1 && cartItems.map((item) => {
            ...
            return (
              <div key={item._id} style={{ backgroundColor: 'white', padding: '20px', borderRadius: '15px', boxShadow: '0 4px 10px rgba(0,0,0,0.05)', display: 'flex', alignItems: 'center', justifyContent: 'space-between', flexWrap: 'wrap', gap: '15px' }}>
                ...
                <div style={{ flex: '1', minWidth: '200px', padding: '0 15px' }}>
                  ...
                  {discount > 0 ? (
                    <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'flex-start', gap: '2px' }}>
                      <span style={{ textDecoration: 'line-through', color: '#999', fontSize: '0.9rem' }}>{originalPrice} ج.م</span>
                      <div style={{ display: 'flex', alignItems: 'center', gap: '10px' }}>
                        <span style={{ color: '#d4af37', fontWeight: 'bold', fontSize: '1.1rem' }}>{finalPrice} ج.م</span>
                        <span style={{ backgroundColor: '#ffebee', color: '#c62828', fontSize: '0.8rem', padding: '2px 8px', borderRadius: '4px', fontWeight: 'bold' }}>خصم {discount}% 🔥</span>
                      </div>
                    </div>
                  ) : (
                    <span style={{ color: '#d4af37', fontWeight: 'bold', fontSize: '1.1rem' }}>{originalPrice} ج.م</span>
                  )}
                </div>
                ...
                {/* زر الحذف */}
                <button type="button" onClick={() => onRemove(item)} style={{ background: 'none', border: 'none', color: '#e74c3c', cursor: 'pointer', fontSize: '1.5rem', padding: '10px' }}>🗑️</button>
              </div>
            )
          })}
        </div>
        ...
        {cartItems.length >= 1 && (
          <div style={{ marginTop: '40px', backgroundColor: 'white', padding: '30px', borderRadius: '20px', boxShadow: '0 5px 20px rgba(0,0,0,0.05)' }}>
            ...
            <div style={{ textAlign: 'center' }}>
              <button type="button" className="whatsapp-checkout-btn" onClick={handleCheckout} style={{ width: '100%', padding: '18px', backgroundColor: '#25D366', color: 'white', border: 'none', borderRadius: '12px', fontSize: '1.1rem', fontWeight: 'bold', cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '10px', boxShadow: '0 4px 15px rgba(37, 211, 102, 0.4)' }}>
                 📱 إتمام الطلب عبر واتساب
              </button>
            </div>
            
            {/* ❌ تم حذف زر العودة المكرر من هنا لأن _app.js يظهره تلقائياً */}
