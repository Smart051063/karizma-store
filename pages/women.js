...
export default function Women({ products }) {
  return (
    <div style={{ minHeight: '100vh', backgroundColor: '#f9f9f9', padding: '40px 20px', direction: 'rtl', fontFamily: 'Arial' }}>
      <Head>
        <title>عطور نسائية | كاريزما للعطور</title>
        <meta name="description" content="تسوقي أرقى العطور النسائية من كاريزما. تشكيلة فريدة تناسب ذوقك الرفيع." />
      </Head>

      <h1 style={{ textAlign: 'center', marginBottom: '40px', color: '#333' }}>🌸 عطور نسائية</h1>
        ...
        {products.map((product) => (
          <Link href={`/product/${product.slug.current}`} key={product._id} style={{ textDecoration: 'none' }}>
            <div className="product-card" style={{ width: '250px', backgroundColor: 'white', borderRadius: '15px', overflow: 'hidden', boxShadow: '0 4px 10px rgba(0,0,0,0.05)', transition: '0.3s' }}>
              <div style={{ position: 'relative', height: '250px', backgroundColor: '#f4f4f4' }}>
                 {/* عرض خصم إن وجد */}
                 {product.discount > 0 && (
      ...
      {/* ❌ تم حذف زر العودة من هنا لأنه موجود تلقائياً في _app.js */}
...
export const getStaticProps = async () => {
  const query = `*[_type == "product" && category == 'women'] | order(_createdAt desc) {
    _id,
    name,
    price,
    discount,
