...
export default function GulfPage() {
  ...
  useEffect(() => {
    // 👇 استعلام لجلب منتجات قسم "gulf" أو "Gulf"
    const query = `*[_type == "product" && (category == "gulf" || category == "Gulf")]{
      _id,
      name,
      price,
  ...
  return (
    <div style={{ padding: '20px', direction: 'rtl', textAlign: 'center' }}>
      ...
      {/* عنوان مميز للقسم الخليجي */}
      <h1 style={{ color: '#d4af37', marginBottom: '30px' }}>🏰 عطور خليجية فاخرة</h1>
      ...
      <div style={{ display: 'flex', flexWrap: 'wrap', justifyContent: 'center', gap: '20px', marginTop: '20px' }}>
        {products.length > 0 ? (
          ...
          <p>جاري تحميل العطور الخليجية... ⏳</p>
        )}
      </div>
    </div>
  );
